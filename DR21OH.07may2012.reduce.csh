#!/bin/csh
# reduce.csh  16-may-2012
# 28-apr-2012: separates wides and narrows before fitting passband
# 01-may-2012: write out leakage log files for Chat
# 16-may-2012; deal with narrowband data


# inputs: $RAW1, $RAW2, etc -> concatenate into $RAW
# fit xyphase to $RAW -> rewrite to 'wide.xy'
# fit passband to 'wide.xy' -> rewrite to 'wide.pb'
# ... also segregate MWC349 spectra, if present, to MWC349.pb
# avg into 6 chans -> 'wide.av'
...
# ... tsys erratic on C8 LCP, C12 RCP
...
# ------------------------------------------------------------------
# C8 bad after 13:00 UT
  foreach FILE (wide.xy win7.xy win15.xy)
    uvflag vis=$FILE select='ant(8),time(13:00,23:59)' flagval=flag
  end
# ------------------------------------------------------------------
  ...
  # Segregate MWC349 channel data, if present, for later processing of H30alpha recomb line
...
goto end
    ...
    rm -r junk
    cp -r wide.av junk
    uvlist options=var,full vis=junk | grep evector
	gpcal vis=junk options=circular,qusolve,noxy,nopass flux=$LEAKFLUX interval=0.5 \
	  refant=$REFANT select='source('$LEAKCAL')' line=chan,1,1,6 #> gpcal.DSB.log
    gpplt vis=junk options=polarization device=/xs nxy=2,2 yrange=0,0.2 
    uvflux vis=junk select='source('$LEAKCAL')' line=chan,1,1,6 stokes=I,Q,U,V \
	  options=uvpol,nopass

    puthd in=junk/evector value=0 type=double
    uvlist options=var,full vis=junk | grep evector
	gpcal vis=junk options=circular,qusolve,noxy,nopass flux=$LEAKFLUX interval=0.5 \
	  refant=$REFANT select='source('$LEAKCAL')' line=chan,1,1,6 #> gpcal.DSB.log
    gpplt vis=junk options=polarization device=/xs nxy=2,2 yrange=0,0.2 
...
# just putting in a different evector makes no difference at all - get exactly the
# same leakages and same calibrator position angle

    rm -r junk2
    uvredo options=chi,nopol,nopass,nocal vis=junk out=junk2 
    uvlist options=var,full vis=junk2 | grep evector
goto end

    uvlist options=list vis=junk 
    uvlist options=list vis=junk2 
# parallactic angle is unchanged

	gpcal vis=junk2 options=circular,qusolve,noxy,nopass flux=$LEAKFLUX interval=0.5 \
	  refant=$REFANT select='source('$LEAKCAL')' line=chan,1,1,6 #> gpcal.DSB.log
    gpplt vis=junk2 options=polarization device=/xs nxy=2,2 yrange=0,0.2 
    uvflux vis=junk2 select='source('$LEAKCAL')' line=chan,1,1,6 stokes=I,Q,U,V \
	  options=uvpol,nopass
goto end

    #smauvplt vis=junk select='source('$LEAKCAL')' axis=time,parang device=/xs options=nobase
    #$<
    #smauvplt vis=junk2 select='source('$LEAKCAL')' axis=time,parang device=/xs options=nobase
    #$<

	gpcal vis=junk2 options=circular,qusolve,noxy,nopass flux=$LEAKFLUX interval=0.5 \
	  refant=$REFANT select='source('$LEAKCAL')' line=chan,1,1,6 #> gpcal.junk.log
    gpplt vis=junk2 options=polarization device=/xs nxy=2,2 yrange=0,0.2 
    uvflux vis=junk2 select='source('$LEAKCAL')' line=chan,1,1,6 stokes=I,Q,U,V \
	  options=uvpol,nopass
goto end
...
goto end
  ...
  # create wide.lsb, fit LSB leakages
    rm -r wide.lsb
  ...
  # create wide.usb, fit USB leakages
    rm -r wide.usb
...
# >>> copy leakages derived above to master leak directory, find best avg leakages <<< #
...
goto end
  ...
  # Copy wide.av gains to win15 data, rewrite to apply
...
# -----------------------------------...........-------------------------------
  ...
  # Fit passband (and 1 gain) to grand avg of win15 data
...
goto end
  ...
  # Rewrite the data one more time to apply gain and passband 
    rm -r win15.cal
    uvcat vis=win15.tmp out=win15.cal options=nopol

  # Copy the leakage from wide.usb - this completes calibration of win15 data
...
goto end
  ...
  # Copy wide.av gains to win7 data, rewrite to apply
    ...
    uvplt vis=win7.tmp axis=time,phase device=/xs line=chan,1,1,191 average=10 \
	  select='-source('$SRC')' options=nocal,nopass,nopol nxy=4,4

  # Fit passband (and 1 gain) to grand avg of win7 data
...
goto end
  ...
  # Copy the leakage from wide.lsb - this completes calibration of win7 data
...
goto end
  ...
  # Cleanup
    rm -r win7.xy win7.tmp sio.sl sio.mp
