...
def ampPhsPlot( infile, x0=0., y0=0., distIn=6.2, ExtendPhase=False, Angle=False ) :
    xa,ya,amp,phs = readData( infile )
    theta = numpy.arctan( numpy.sqrt(numpy.power(xa-x0,2)+numpy.power(ya-y0,2))/distIn )
  ...
  # extend phases through +/- 180 degrees
    if ExtendPhase :
  ...
  # create interpolated arrays for smoother plots
    delta = (y[1]-y[0])/5.
    xi = numpy.arange( x.min(),x.max(),delta )
    yi = numpy.arange( y.min(),y.max(),delta )
    xmin = x.min()
    ...
    ymin = y.min()
    ymax = y.max()
    ampi = griddata( x,y,amp, xi,yi, interp='linear' )
